jQuery(function($){
	...
	var _pb_calendar = (function(target_, options_){
		...
		this._target.append('<div class="top-frame">' + 
			'<div class="year-month-frame control-frame">'+
		...
		this._target.append('<div class="calendar-head-frame">' +
			'<div class="row row-dayname">' +
				'<div class="col col-dayname holiday"><div class="inner">SUN</div></div>' +
				'<div class="col col-dayname"><div class="inner">MON</div></div>' +
				'<div class="col col-dayname"><div class="inner">TUE</div></div>' +
				'<div class="col col-dayname"><div class="inner">WED</div></div>' +
				'<div class="col col-dayname"><div class="inner">THU</div></div>' +
				'<div class="col col-dayname"><div class="inner">FRI</div></div>' +
				'<div class="col col-dayname holiday"><div class="inner">SAT</div></div>' +
			'</div>' +
		'</div>');
	...
	_pb_calendar.prototype.update_view = (function(){
		...
		for(var day_index_= 0; day_index_ <= day_count_; ++day_index_){
			...
			var target_yyyymmdd_num_ = parseInt(target_moment_.format("YYYYMMDD"));
			...
			if(target_yyyymm_num_ < yyyymm_num_){
				...
				if(target_yyyymmdd_num_ == moment().format("YYYYMMDD")){
					col_class_name_ = "today";
				}else if(target_yyyymmdd_num_ < moment().format("YYYYMMDD")){
					col_class_name_ = "before-day";
				}
			}
			...
			result_html_ += '<div class="col '+col_class_name_+'" data-day-yyyymmdd="'+target_moment_.format("YYYYMMDD")+'"><div class="inner">';
			...
			if(this._options['day_selectable']){
				...
				result_html_ += target_moment_.format("D");
			}

			result_html_ += '<div class="schedule-dot-list"></div>';
			
			
			result_html_ += '</div></div>';
		...
		this._schedule_list(function(schedule_list_){
			...
			$.each(schedule_list_, function(yyyymmdd_, schedule_data_){
				var schedule_dot_list_el_ = that_._body_frame.find("[data-day-yyyymmdd='"+yyyymmdd_+"'] > .inner .schedule-dot-list");

				$.each(schedule_data_, function(){
					var schedule_dot_item_ = $('<span class="schedule-dot-item '+this.style+'" data-schedule-id="'+this['ID']+'"></span>'); 
